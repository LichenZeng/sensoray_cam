...
/*
 * begin backward compatibility code.
 */
#if LINUX_VERSION_CODE < KERNEL_VERSION(2, 6, 24)
#error "Please try driver in linuxtv path"
#endif

//TODO: verify exact version.  Know it was changed at 2.6.37
#define KERNEL_VERSION_VMALLOCINIT_CHANGED       KERNEL_VERSION(2, 6, 37)
#define KERNEL_VERSION_V4L2TIMESTAMP KERNEL_VERSION(3, 9, 0)
#define KERNEL_VERSION_V4L2CONTROLS  KERNEL_VERSION(3, 10, 0)
...
static inline struct s2255_dev *get_dev_from_channel(struct s2255_channel *channel)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
	struct v4l2_device *v4l2_dev = channel->vdev.v4l2_dev;
	return container_of(v4l2_dev, struct s2255_dev, v4l2_dev);
#else
	return channel->dev;
#endif
...
#define S2255_CUR_DSP_FWVER     10104 /* coded 1.1.2*/
...
#define S2255_IOC_GET_SN _IOR('v', BASE_VIDIOC_PRIVATE+0, struct s2255_sn)
...
#define S2255_IOC_GET_FW _IOR('v', BASE_VIDIOC_PRIVATE+1, struct s2255_fw_ver)
...
/* image formats.  */
/* make sure JPEG is defined last */
...
static int s2255_got_frame(struct s2255_channel *channel, int jpgsize)
{
	...
	struct s2255_dev *dev = get_dev_from_channel(channel);
			   ...
			   struct v4l2_capability *cap)
{
	...
	cap->version = KERNEL_VERSION(1, 22, 0);
...
#endif
{
	...
	if (ival & V4L2_STD_525_60) {
		...
		if (mode.format != FORMAT_NTSC) {
			...
			mode.fdec = FDEC_1;
			fh->channel->width = LINE_SZ_4CIFS_NTSC;
			fh->channel->height = NUM_LINES_4CIFS_NTSC * 2;
		}
	} else if (ival & V4L2_STD_625_50) {
		dprintk(4, "%s 50 Hz\n", __func__);
		if (mode.format != FORMAT_PAL) {
			mode.restart = 1;
			mode.format = FORMAT_PAL;
			mode.fdec = FDEC_1;
			fh->channel->width = LINE_SZ_4CIFS_PAL;
			fh->channel->height = NUM_LINES_4CIFS_PAL * 2;
		}
	} else {
		ret = -EINVAL;
		goto out_s_std;
			 ...
			 struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_S2255_COLORFILTER:
		...
		break;
#if LINUX_VERSION_CODE > KERNEL_VERSION_V4L2CONTROLS
	case V4L2_CID_JPEG_COMPRESSION_QUALITY:
		channel->jpegqual = ctrl->value;
		return 0;
#endif
	default:
		return -EINVAL;
			 ...
			 struct v4l2_streamparm *sp)
{
	...
	if (channel->cap_parm.capturemode
		...
		return -EBUSY;
	channel->cap_parm.capturemode = sp->parm.capture.capturemode;
...
static int s2255_read_reg_burst(struct s2255_dev *dev, u8 dev_addr, u16 reg_addr, u8 *data, u8 datalen);

#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 10, 0)
// 
static long vidioc_default(struct file *file, void *__fh, bool valid_prio,
...
static long vidioc_default(struct file *file, void *__fh, int cmd, void *arg)
#else
static int vidioc_default(struct file *file, void *__fh, int cmd, void *arg)
#endif
{
	struct s2255_channel *channel = video_get_drvdata(video_devdata(file));
	struct s2255_dev *dev = get_dev_from_channel(channel);
	struct s2255_sn *sn = arg;
	struct s2255_fw_ver *fw = arg;
	u8 *outbuf;
	int rc;
	switch (cmd) {
	case S2255_IOC_GET_SN:
		outbuf = kzalloc(16 * sizeof(u8), GFP_KERNEL);
		if (outbuf == NULL)
			return -ENOMEM;
		memset (sn, 0, sizeof(struct s2255_sn));
		rc = s2255_read_reg_burst(dev, 0xa2, 0x1ff0, outbuf, 16);
		if (rc < 0) {
			kfree(outbuf);
			return rc;
		}
		...
		sn->sn = (outbuf[12] << 24) + (outbuf[13] << 16) + (outbuf[14] << 8) + outbuf[15];
		sn->month = outbuf[9];
		sn->day = outbuf[10];
		sn->year = outbuf[11];
		kfree(outbuf);
		break;
	case S2255_IOC_GET_FW:
		fw->usb_fw = dev->usb_fw_ver;
		fw->dsp_fw = dev->dsp_fw_ver;
		break;
	default:
		return -EINVAL;
	} 
...
#if LINUX_VERSION_CODE < KERNEL_VERSION_VMALLOCINIT_CHANGED
...
#else
static int s2255_release(struct inode *inode, struct file *file)
#endif
{
	struct s2255_fh *fh = file->private_data;
	struct s2255_dev *dev = fh->dev;
...
static int s2255_mmap_v4l(struct file *file, struct vm_area_struct *vma)
{
	...
	// workaround deadlock bug introduced in 3.11
	mutex_lock(&q->vb_lock);
...
{
	struct s2255_channel *channel = video_get_drvdata(vdev);
	struct s2255_dev *dev = get_dev_from_channel(channel);
	dprintk(4, "%s, chnls: %d \n", __func__,
...
#else
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 27)
        channel->vdev.parent = &dev->interface->dev;
#else
        channel->vdev.dev = &dev->interface->dev;
#endif
#endif
		       ...
		       const struct usb_device_id *id)
{
	...
	if (*pdata != S2255_FW_MARKER) {
		...
		printk(KERN_INFO "s2255 dsp fw version[%x] %d.%02d.%02d\n", *pRel,
               *pRel / 10000, (*pRel % 10000) /100, *pRel % 100);
...
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 29)
	// intfdata is v4l2_dev
	struct s2255_dev *dev = to_s2255_dev(usb_get_intfdata(interface));
#else
	// intfdata is dev
	struct s2255_dev *dev = usb_get_intfdata(interface);
#endif
	int i;
