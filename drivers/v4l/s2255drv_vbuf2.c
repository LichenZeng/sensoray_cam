...
static void s2255_got_frame(struct s2255_vc *vc, int jpgsize)
{
	...
	buf->vb.v4l2_buf.timestamp = timevalue;
#endif
...
static void s2255_wait_finish(struct vb2_queue *vq)
{
	struct s2255_vc *vc = vb2_get_drv_priv(vq);
	mutex_lock(&vc->vb_lock);
}

static void s2255_wait_prepare(struct vb2_queue *vq)
{
	struct s2255_vc *vc = vb2_get_drv_priv(vq);
	mutex_unlock(&vc->vb_lock);
}
#endif
...
#else
	 .wait_prepare = s2255_wait_prepare,
	 .wait_finish = s2255_wait_finish,
#endif
};
...
#else
static int vidioc_s_std(struct file *file, void *priv, v4l2_std_id *i2)
#endif
{
	struct s2255_vc *vc = video_drvdata(file);
	struct s2255_mode mode;
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 6, 0)
static int s2255_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct s2255_vc *vc = video_drvdata(file);
	int ret;

	ret = vb2_mmap(&vc->vb_vidq, vma);
	return ret;
}

static ssize_t s2255_read(struct file *file, char __user *buf,
			      size_t count, loff_t *ppos)
{
	struct s2255_vc *vc = video_drvdata(file);
	return vb2_read(&vc->vb_vidq, buf, count, ppos, file->f_flags & O_NONBLOCK);
}
...
static unsigned int s2255_poll(struct file *file, struct poll_table_struct *wait)
{
	...
	ret = vb2_poll(&vc->vb_vidq, file, wait);
	return ret;
}

static int s2255_streamon(struct file *file, void *priv, enum v4l2_buf_type i)
{
	struct s2255_vc *vc = video_drvdata(file);
	int ret;
	
	ret = vb2_streamon(&vc->vb_vidq, i);
	return ret;
}

static int s2255_streamoff(struct file *file, void *priv, enum v4l2_buf_type i)
{
	struct s2255_vc *vc = video_drvdata(file);
	int ret;

	ret = vb2_streamoff(&vc->vb_vidq, i);
	return ret;
}
...
{
	struct s2255_vc *vc = video_drvdata(file);
	return vb2_reqbufs(&vc->vb_vidq, p);
}
...
{
	struct s2255_vc *vc = video_drvdata(file);
	return vb2_querybuf(&vc->vb_vidq, p);
}
...
{
	struct s2255_vc *vc = video_drvdata(file);
	return vb2_dqbuf(&vc->vb_vidq, p, file->f_flags & O_NONBLOCK);
}
...
#else
	.vidioc_streamon = s2255_streamon,
	.vidioc_streamoff = s2255_streamoff,
#endif
